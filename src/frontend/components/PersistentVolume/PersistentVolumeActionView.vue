  ...
  return [
    {
      ...
      action: async () => {
        await DrawerHelper.instance.drawer(drawer).showResourceEditor({
          ...
          ns: props.persistentVolume.metadata.namespace,
          name: props.persistentVolume.metadata.name,
        })
      },
    },
    {
      ...
      icon: Trash,
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `Namespace：${props.persistentVolume.metadata.name}`, async () => {
          await K8sService.playService.persistentVolumeControllerDelete({ requestBody: [`${props.persistentVolume.metadata.namespace}`] })
          drawer.close()
        }),
    },
