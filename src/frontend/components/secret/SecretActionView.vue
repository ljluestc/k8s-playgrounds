...
function getOptions(): ActionMenuOption[] {
  return [
    ...
    },
    {
      ...
      icon: Trash,
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.secret.metadata.namespace}/${props.secret.metadata.name}`, async () => {
          await K8sService.playService.secretControllerDelete({ requestBody: [`${props.secret.metadata.namespace}/${props.secret.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
