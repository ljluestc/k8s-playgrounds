...
const props = defineProps({
  vw: V1ValidatingWebhookConfiguration,
  isDropdown: Boolean,
})
  ...
  return [
    {
      ...
      action: async () => {
        await DrawerHelper.instance.drawer(drawer).showResourceEditor({
          resType: ResType.ValidatingWebhookConfiguration,
          ns: props.vw.metadata.namespace,
          name: props.vw.metadata.name,
        })
      },
    },
    {
      ...
      icon: Trash,
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.vw.metadata.name}`, async () => {
          await K8sService.playService.validatingWebhookControllerDelete({ requestBody: [`${props.vw.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
